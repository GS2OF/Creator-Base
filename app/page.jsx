...
export default function Page() {
  ...
  const [copied, setCopied] = useState(false); // Link kopiert Feedback
  ...
  // Tiefenlink: #plattform-match öffnet das Modal; Hash wird gepflegt.
  const openMatch = () => {
    setMatchOpen(true);
    setMatchStep(1);
    setMatchResult(null);
    ...
    if (typeof window !== "undefined") {
      history.replaceState(null, "", `${window.location.pathname}${window.location.search}#plattform-match`);
    }
  };
  const closeMatch = () => {
    setMatchOpen(false);
  ...
  useEffect(() => {
    const handleHash = () => {
      if (window.location.hash === "#plattform-match") {
        ...
        if (matchOpen) setMatchOpen(false);
      }
    };
    handleHash();
  ...
  const copyMatchLink = async () => {
    ...
    const url = `${window.location.origin}${window.location.pathname}${window.location.search}#plattform-match`;
    try {
      await navigator.clipboard.writeText(url);
      setCopied(true);
      setTimeout(() => setCopied(false), 1500);
    } catch {
      // rudimentärer Fallback
          ...
          <div className="relative z-10 h-full overflow-y-auto">
            <div className="min-h-full flex items-start md:items-center justify-center p-4">
              <div className="mx-4 md:mx-0 rounded-2xl border border-white/10 bg-[#0f0f14] shadow-2xl overflow-hidden w-full md:w-[880px] max-h-[calc(100dvh-2rem)] overflow-y-auto">
                {/* Header */}
                <div className="px-5 py-4 border-b border-white/10 flex flex-wrap items-center gap-2 justify-between">
                  <div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded border border-white/20 bg-white/10 hover:bg-white/20 px-3 py-1.5 text-sm"
                      title="Link kopieren"
                    >
                      <Copy className="size-4" />
                      {copied ? "Kopiert!" : "Link kopieren"}
                    </button>
                    <button onClick={closeMatch} className="text-white/70 hover:text-white flex items-center gap-1 px-3 py-1.5 rounded border border-white/20 bg-white/10">
                ...
                {matchStep === 1 && !matchLoading && (
                  <form onSubmit={submitPlatformMatch} className="px-5 pb-5 grid gap-4">
                    ...
                    <div className="flex items-center justify-end gap-2 pt-2">
                      <button type="button" onClick={closeMatch} className="px-4 py-2 rounded bg-white/10 border border-white/20">Abbrechen</button>
                ...
                {matchStep === 2 && matchResult && (
                  <div className="px-5 pb-6">
                    ...
                    {/* Footer-Actions kompakt (mobile-freundlich) */}
                    <div className="mt-5 flex flex-wrap items-center justify-end gap-2">
                      ...
                      <button
                        onClick={copyMatchLink}
                        className="px-4 py-2 rounded border border-white/20 bg-white/10 hover:bg-white/20 inline-flex items-center gap-1"
                      >
                        <Copy className="size-4" /> {copied ? "Link kopiert!" : "Link kopieren"}
                      </button>
                    </div>
                  </div>
                )}
